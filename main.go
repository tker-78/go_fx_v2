...
func main() {
	...
	*/
	...
	startTime := time.Date(2021, 04, 01, 00, 00, 00, 00, time.UTC)
	endTime := time.Date(2022, 06, 01, 00, 00, 00, 00, time.UTC)
	df, err := models.GetCandlesByBetween(startTime, endTime, "1d")
	if err != nil {
		log.Println(err)
	}
